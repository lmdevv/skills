---
name: commit
description: "Use when the user runs /commit or asks the agent to commit changes. Enforces conventional-style commits with co-author attribution."
---
...
## New Thread (no changes made by you)

If this is a new thread and you did **not** make any changes:

1. `git status --short`
2. `git diff --stat` and `git diff` (staged + unstaged) - inspect thoroughly.
3. `git log -5 --oneline` - match repo style.
4. Stage relevant files.
5. Commit with conventional title and optional bullet body.

**Do NOT add a Co-authored-by trailer.** You did not author these changes.
...
```
Co-authored-by: <provider>/<model-name> <noreply@<provider>.ai>
```
